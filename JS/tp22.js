...
for (let k = 0; k < nbrEtudiant; k++) {
  ...
  if (listeEtudiant[k].pays.toLowerCase() === "chine") {
    origineChinois += 1;
  }
  //La liste des personnes dont le prénom termine par "a"
  if (
    listeEtudiant[k].prenom[
      listeEtudiant[k].prenom.length - 1
    ].toLowerCase() === "a"
  ) {
  ...
  if (
    listeEtudiant[k].pays.toLowerCase() === "russie" &&
    listeEtudiant[k].genre.toLowerCase() === "m"
  ) {
  ...
  if (
    listeEtudiant[k].prenom[0].toLowerCase() === "k" &&
    listeEtudiant[k].genre.toLowerCase() === "f"
  ) {
    listeFemmePrenomK.push(listeEtudiant[k]);
  }
  //La liste de tous les hommes dont les noms commencent par M
  if (
    listeEtudiant[k].prenom[0].toLowerCase() === "m" &&
    listeEtudiant[k].genre.toLowerCase() === "m"
  ) {
...
console.log(
  `Liste des personnes dont le prénom termine par "a" : `,
  prenomEndA
);
console.log(`Liste de tout les noms : `, listeNoms);
console.log(`Liste de tout les hommes russes : `, listeHommeRusse);
console.log(
  `Liste de toutes les personnes dont les noms commencent par K et qui sont des femmes : `,
  listeFemmePrenomK
);
console.log(
  `Liste de toutes les personnes dont les noms commencent par M et qui sont des hommes : `,
  listeHommePrenomM
);
